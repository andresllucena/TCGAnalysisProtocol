...
############### Build gene expression structure for edgeR DEG analysis ############### 
...
for (i in 1:10){
  ...
  y <- cbind(Normal_RNA, StrSample[[i]])  
  # the group 2 will be the one which the results indicates genes up/down-regulated
  # compared to group 1
  
  # Now define the group of each sample
  group <- c()
  group_num <- c(1,2)
  group[1:59]<- unique(group_num)[1] 
  group[60:209]<- unique(group_num)[2] 
  
  
  # Create a DGElist object
  bb <- DGEList(counts=y[1:209], group=group)
  #x[["counts"]] 
  x[[i]] <- bb
  ...
  plotMDS <- plotMDS(x[[i]], top = 500, pch=points[group], col=colors[group], gene.selection = "pairwise", main="Similaridade de expressão entre amostras normais e tumorais (mRNA)") #da os nomes de acordo com os grupos
  ...
  # Choose dots appearance and some of plot legend settings
  ...
  # Make the graph
  plotMD(et[[i]], hl.pch=1, hl.col = colors, status = is.de, main = "Normal x Adenocarcinoma (mRNA)")
  ...
  ############### Extract raw counts of a specific gene  ############### 
  counts <- t(x[[i]]$counts)
  counts <- subset(counts, select=c(ENSG00000177425))
  # Merge a sample group value for each sample
  merged <- cbind(counts, group)
  # Export a table with the data of your gene of interest
  write.table(merged, file = paste0(i, "PAWR.txt", sep = ""),
              sep = "\t",
              row.names = TRUE, col.names = NA, 
              qmethod = c("escape", "double"))
  rm(counts, merged)
...
rm(colors, group, group_name, group_num, points, bb, is.de, x)
...
# make columns seem duplicated so we can rowmean it later
